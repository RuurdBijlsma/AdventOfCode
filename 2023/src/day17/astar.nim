...
iterator path*[G: Graph, N: Node, D: Distance](graph: G, start, goal: N): N =
    ...
    while frontier.size > 0:
        ...
        let recent = recentPath(cameFrom, start, current.node, 3).toSeq
        var illegalPos = (-1, -1)
        if recent.len == 3:
            let distX = abs(recent[0].x - recent[2].x)
            let distY = abs(recent[0].y - recent[2].y)
            if distX == 2:
                illegalPos = (recent[0].x + (recent[0].x - recent[1].x), recent[0].y)
            if distY == 2:
                illegalPos = (recent[0].x, recent[0].y + (recent[0].y - recent[1].y))

        echo "recent path"
        echo recent
        echo &"illegalpos={illegalPos}"

        for next in graph.neighbors(current.node):

            # The cost of moving into this node from the goal
            var cost = current.cost + D( graph.cost(current.node, next) )
            if next == illegalPos:
                cost += 1000
